  ...
  return (
    <div className="fixed z-50 bottom-4 right-4">
      {isOpen ? (
        <Card className="shadow-lg w-80 md:w-96">
          <CardHeader className="p-4 border-b">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="icon" onClick={toggleChat} className="w-8 h-8">
                <X className="w-4 h-4" />
              </Button>
            </div>
          </CardHeader>
          <CardContent ref={chatContainerRef} className="p-4 overflow-y-auto h-80">
            {messages.length === 0 ? (
          ...
          <CardFooter className="p-3 border-t">
            <form onSubmit={handleSubmit} className="flex items-center w-full space-x-2">
              <Input value={input} onChange={(e: any) => setInput(e.target.value)} placeholder="Type your message..." className="flex-1" disabled={isLoading} />
        ...
        <Button onClick={toggleChat} size="icon" className="w-12 h-12 rounded-full shadow-lg">
